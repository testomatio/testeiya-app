...
**Sidebar tree** (`GET /api/files/tree`, `files-tree.ts`): all non-vendor, non-dot, **non-git-ignored** **folders** are shown; **only `*.test.md` files** outside the manual-tests dir; **all files** inside it (and the dir is auto-expanded, even when it lives under `.testeiya/`). The response carries `{ manualTestsDir, isProject, project }`. **Git-ignored paths are hidden** — the walk (and the `workspace-model` classifier) honors every `.gitignore` up the tree via `cli/src/gitignore.ts` (a lazy nested-aware matcher over the `ignore` package), so a repo's `node_modules`-style caches (e.g. a Rails `storage/` blob dir) neither render nor blow the node budget. The surfaced `.testeiya` dir is the one exception (it's git-ignored but deliberately folded — whole — into the **Files** view: the pulled `manual-tests`, user-added context under `code`/`requirements`, and the config files). Views for an overlay repo (code repo + a pulled `.testeiya/manual-tests` cache): **Code** = the source repo only (no `.testeiya`), **Manual** = the tests, **Files** = both together plus all of `.testeiya`. The Files tab is added for a `code` root only when that overlay exists (`listWorkspaceTypes`); a pure code repo shows just `code` (Code already lists everything).

**Manual vs Automated tests.** A pull with `--export-automated` folds the project's *automated* tests into the **same** `*.test.md` suite files as the manual ones, each tagged `type: manual` or `type: automated` inside its `<!-- test` marker (**no `type:` line means manual** — common, so never count manual tests by grepping `type: manual`). The tree does **not** filter on this: one tab lists both. `readSuite` (`files-tree.ts`) parses each block's `type:` onto the node as `testType`, which only drives the row icon. When the manual source holds any automated markdown, `listWorkspaceTypes` sets `label: "tests"` on the manual entry so the tab reads **Tests** instead of the now-inaccurate **Manual**; `WorkspaceTypeEntry.label` overrides the type as the tab caption. The agent slices the two kinds itself — see the `<test-types>` block in `cli/src/prompt/system-prompt.ts`, which teaches it the awk recipe plus the crucial fact that **automated tests here are references, not runnable code** (the real implementation lives in the automation repo; they run via a Testomat.io CI profile).

**Sync** (`POST /api/workspace/sync` `{session, action}`): runs `check-tests` (`cli/src/check-tests.ts`, `npx check-tests[@latest] pull|push -d <dir>`) for the resolved manual-tests dir. **Pulls pass `--export-automated`** (so automated tests come down as markdown too) **except** when the workspace is itself an automation repo — `hasAutomationConfig(cwd)` (playwright/codecept/cypress/wdio/… config or dep) — where the repo's own spec files already *are* the automated tests and markdown copies would duplicate them. Same guard in both pull paths: `api/workspace-sync.ts` and `api/agent-start.ts`. Token resolution: a managed session's `tokens[slug]` → else the linked project's `apiKey` via the connected account (`.testeiya/testeiya.json` + `loadStoredAuth`) → else the folder's own `.env` `TESTOMATIO`. The Workspace sidebar section has Pull/Push buttons (`WorkspaceService.sync`). Sync semantics follow the vendored `cli/skills/Test Management/sync-test-cases-with-tms/SKILL.md` (from `testomatio/skills`).
...
## Skills (external vendors + internal + custom)

Testeiya passes an **explicit `skills[]` array** into `createAgentSession` (`session-factory.ts`), which bypasses the SDK's own skill discovery. The array is `dedupeSkillsByName([...loadBundledSkills(), ...loadCustomSkills(cwd)])`, so a later source (custom last) can **override** an earlier one by reusing its `name`.

The bundled tree **`cli/skills/` is organized by vendor folder** and feeds `loadBundledSkills()`:
1. **External vendors (`cli/skills.yaml`)** — a **flat list of GitHub repos**, one `owner/repo` per line (optionally `owner/repo/tree/<ref>/<subdir>` to pin a ref or point at a subfolder — e.g. the playwright-cli skill is `microsoft/playwright-cli/tree/main/skills/playwright-cli`). **No exceptions — every line is a GitHub repo.** Each source owns **one vendor folder** (the repo name, or the owner when the repo is just `skills`: `testomatio/skills` → `cli/skills/testomatio/`); `bunosh skills:update` replaces **only manifest-owned folders**.
2. **Internal (first-party)** — any folder the manifest does **not** own (canonically **`cli/skills/testeiya/`**) is hand-authored in this repo and **never touched by updates**. Add a skill as `cli/skills/testeiya/<skill>/SKILL.md` (category = "Testeiya"), or nest category subfolders like a vendor would; scaffold one with **`bunosh skills:create <name>`**.
3. **User custom** — folders the user drops in `~/.testeiya/skills` / `<cwd>/.testeiya/skills` (below).

> A skill that lives in an npm package is **never** put in `skills.yaml` (the update only clones GitHub). The `@playwright/cli` package stays a dependency, but only for the browser CLI **tool** (the binary the agent drives) — `hasPlaywrightCli()` gates the browser guidance on it. The playwright-cli **skill** is vendored from GitHub like everything else.

- **Updating** (build time): the **`skillsUpdate`** task in **`cli/Bunoshfile.js`** — run **`bunosh skills:update [vendor]`** from the repo root (via `Bunoshfile.skills.js`) or from `cli/`; **no `cd cli` for `bun run`**. Pass a vendor (folder, owner, or `owner/repo`) to refresh just that one. It resolves each source's SHA via the GitHub API, downloads the tarball (no `git` binary), removes only that source's vendor folder, and copies the selected skills in. **Categories inside a vendor folder are decided by the repo**: if the repo has a Claude-plugin marketplace (`.claude-plugin/marketplace.json`), every plugin is a category subfolder — so `testomatio/skills` → `testomatio/{test-management,test-automation,explorbot}/<skill>/`. A repo with **no marketplace** puts skills **directly under the vendor folder** and the vendor is the category (`codeceptjs/<skill>/` → **Codeceptjs**). A skill in the repo but **not in any plugin is skipped** (the update logs it). A repo that is **one skill** (its `SKILL.md` is at the root) is vendored **flat** as `cli/skills/<vendor>/SKILL.md` (`playwright-best-practices-skill/`, `playwright-cli/`). Folder names are slugified; `loadBundledSkills()` prettifies them back into category labels. SHAs + owned folders are pinned in `cli/skills.lock.json` (a folder on disk that the lock doesn't own is treated as internal — the update refuses to clobber it). Edit the manifest + re-run to add/update/swap; **`bunosh skills:list`** shows the tree by vendor (internal folders marked). The tree + manifest ship via `cli/package.json` `files` and the `electrobun.config.ts` copy map; `resolveBundledSkillsDir()` (`project-dir.ts`) probes for the tree like `resolveStaticDir()` finds `out/`. **On release** both CI jobs re-run the update (`.github/workflows/release.yml`) so shipped external skills are always current — the manifest-owned folders are a dev-time cache; internal folders are source, not cache.

**Custom (user) skills:** a *skill* is a folder containing a `SKILL.md` (YAML frontmatter `name` + `description`, then the body). Drop or **symlink** one into the global `~/.testeiya/skills/` (`CUSTOM_SKILLS_DIR`) — applies to every session — or a workspace's `<cwd>/.testeiya/skills/` (`projectSkillsDir(cwd)`) for a per-project skill. Custom skills are **flat** (no vendor/category folders — unlike the bundled tree): each direct subfolder with a `SKILL.md` is a skill; `loadCustomSkills` follows symlinks (so a skill can be linked in) and ignores non-skill entries.

- **Loader:** `cli/src/skills.ts` — `loadBundledSkills` (walks `cli/skills/<vendor>/` via `readVendorDir`: a root `SKILL.md` = one flat skill; a child folder with `SKILL.md` = a skill with the vendor as category; a child folder without = a category subfolder), `loadCustomSkills(cwd?)`, `hasPlaywrightCli()` (browser-tool gate), `ensureCustomSkillsDir()`, `dedupeSkillsByName()`; all reuse the `readSkill` / `readSkillsDir` choke-points. `readSkill` uses the frontmatter `name` only when it's a valid slug, else the folder name (so a mention is always a clean token).
...
### The `testomatio-docs` skill (vendored product documentation)

`cli/skills/testeiya/testomatio-docs/` is an **internal skill carrying data**: the whole Testomat.io documentation as searchable markdown, so the agent answers platform questions from the docs instead of memory. It holds `docs/` (the pages, mirroring `src/content/docs` in the source repo), a generated `INDEX.md` (`file — title — public URL — description`, the thing to grep first), and `docs.lock.json` (the pinned source commit). The agent reaches them as `skill://testomatio-docs/<path>`, which the SDK auto-resolves to real paths in `bash`/`read`.

- **Updating:** the **`docsUpdate`** task in **`cli/Bunoshfile.js`** — run **`bunosh docs:update [--force]`** from the repo root (via `Bunoshfile.docs.js`). It resolves `testomatio/docs@HEAD` via the GitHub API, lists the tree, and fetches **only** the `.md`/`.mdx` under `src/content/docs` over raw.githubusercontent — **never the tarball**: the repo carries ~760MB of screenshots against ~1.4MB of markdown. Retired (`not-in-use/`) and stray `"<name> copy"` folders are excluded, and navigation-only stubs (a bare `<DirectoryLinks />`) are dropped, so ~140 of 162 pages land. A re-run on the same commit is a no-op unless `--force`. **On release both CI jobs re-run it** (`.github/workflows/release.yml`) — the committed markdown is a dev-time cache, like the skills tree and the models catalog. It ships automatically via `cli/package.json` `files` (`skills`) + the `electrobun.config.ts` copy map.
- **Why the skill exists in this shape:** Testeiya is a *standalone client* to the platform the docs describe, so `SKILL.md` frames every answer with three rules — every documented feature works; the interface in the docs is the **web app's**, not Testeiya's (interface answers go to the project's host: `.testeiya/testeiya.json` `baseUrl` → `TESTOMATIO_URL` → `https://app.testomat.io`); and actions should be *performed* via the Testomat.io MCP tools or `check-tests` rather than described as clicks. The `<rules>` block in `cli/src/prompt/system-prompt.ts` points at the skill so those questions route through it.
...
`cli/src/file-log.ts` (`initFileLog`) boots the **[evlog](https://evlog.dev)** pipeline on **every** process start, on all four surfaces (desktop, `serve:app`, the `next dev` agent half, terminal CLI). The app log is **NDJSON**, one `<yyyy-MM-dd>.jsonl` per day (7 files kept, evlog fs drain) — `jq`-queryable. Every line is a structured wide event with `timestamp`/`level`/`service`/`environment` plus either a `tag` or a `channel`: `tag:"start"` (pid/mode/runtime), `tag:"config"` (resolved provider/model/permissions + env presence summary, **no keys**), `tag:"console"` (every teed `console.*` line — the `[session]`/`[api]`/`[testomatio→]` terminal output, third-party dep output, `uncaughtException`); `channel:"api"` (one event per `/api` request: method/path/status/durationMs, noisy polls like `/api/playwright/status` suppressed), `channel:"testomatio"` (outbound REST with truncated bodies), `channel:"ai"` (per-LLM-call events incl. tokens + `session:<id>` — the Langfuse session id, feed it to `bun run debug:trace session:<id>`), `channel:"check-tests"`, and `channel:"prompt"` — **one wide event per agent prompt** (model, calls, retries, durationMs, outputTokens, peakContext, session, error) for questions like "prompts with retries > 0". evlog runs `silent` with `jwt`/`bearer` redaction; the same drain feeds the in-memory ring buffer (the snapshot's source) and the Debug-panel SSE. It is **not** a full transcript; the deep per-session detail (prompts, tool IO, generations) lives in **Langfuse**, not on disk. Unlike the snapshot, it's written as things happen, so it's the **first stop when the server crashed or the UI never connected**. The current day's file path is published in `~/.testeiya/server.json` (`logFile`).

### The debug snapshot (server + browser, one JSON)

`GET /api/debug/snapshot?session=<id>` merges everything into one dump. The **server** side reads the evlog memory ring buffer (`cli/src/debug-bus.ts` `buildSnapshot`): outbound Testomat.io REST (`server.requests`), LLM events (`server.ai`), `check-tests` runs, the app-server's own stdout (`server.console`, from the console tee), plus the raw `server.api` request events and `server.prompts` per-prompt wide events. The **browser** pushes what the server can't see via `POST /api/debug/report` — its unified `/api/*` + agent-WS log, captured `console.error`/`warn` + uncaught errors, a MobX store snapshot (`client.entries` / `client.store` / `client.meta`), and a **UI layout map** (`client.layout`). The client reports **errors always** (even with the Debug panel closed) and a full snapshot on load, on panel-open, and every 15s while the panel is on (`lib/services/debug-log-service.ts`).
...
- `file-log.ts` — boots the evlog pipeline (shared by all surfaces): NDJSON `~/.testeiya/logs/<date>.jsonl` fs drain (7 files), memory ring buffer, Debug-panel SSE drain, console tee (see "Debugging")
...
- `~/.testeiya/logs/` — persistent NDJSON app log (`cli/src/file-log.ts`, evlog): `<yyyy-MM-dd>.jsonl` per day (start/config headers, teed console, `api`/`testomatio`/`ai`/`prompt`/`check-tests` wide events incl. the Langfuse `session:<id>` + crashes). 7 daily files kept; survives crashes. Deep per-session detail lives in Langfuse, not on disk. See "Debugging".
