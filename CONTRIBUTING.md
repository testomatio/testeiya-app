...
- The prompt is one readable file, `prompt/system-prompt.ts`. Keep it that way: text first, a small function only where a run genuinely differs (how Testomat.io is reached, the answer contract).
- **Only describe what this CLI has.** It cannot open a browser and has nobody to ask, so guidance about driving a browser, asking the user, or a chat app's widgets does not belong here — an agent told about a tool it does not have wastes turns discovering that. The desktop app keeps its own prompt.
- Detail that is not needed on every turn — a file format, a counting recipe, a cheat sheet — goes to a skill in [testomatio/skills](https://github.com/testomatio/skills), and the prompt names the skill in one line.
