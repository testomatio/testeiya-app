...
### Windows (x64)
1. Download **`*-win-x64-Testeiya-Setup.zip`** from the assets below and extract it.
2. Run **`Testeiya-Setup.exe`**.
3. If Microsoft Defender SmartScreen appears, click **More info → Run anyway**.

### macOS (Apple Silicon)
1. Download **`*-macos-arm64-Testeiya.dmg`**, open it, and drag **Testeiya** into **Applications**.
...
### Linux (x64)
1. Download **`*-linux-x64-Testeiya-Setup.tar.gz`**.
2. Extract it and run the installer:
   ```bash
   tar -xzf *-linux-x64-Testeiya-Setup.tar.gz
   ./installer
   ```
   It installs the app to `~/.local/share/` and adds a desktop shortcut / menu entry.
