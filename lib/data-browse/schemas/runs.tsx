...
export function buildRunsSchema(opts: {
  ...
  const tableSchema = createTableSchema({
    ...
    kind: col.enum(RUN_KINDS).label("Kind").hidden(),
    labels: col
    ...
    unfinished: col.boolean().label("Unfinished").hidden(),
    archived: col.boolean().label("Archived").hidden(),
    in_group: col.boolean().label("In group").hidden(),
