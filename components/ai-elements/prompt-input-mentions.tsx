...
export function useFileMentions({ text, setText, items }: UseFileMentionsArgs): FileMentions {
  ...
  const activeMention = useMemo(() => {
    ...
    if (token !== `@${mention.query}`) return null;
    return mention;
  }, [text, mention]);

  const filtered = useMemo(() => {
    if (!activeMention) return [];
    return filterItems(items, activeMention.query);
  }, [items, activeMention]);
...
function filterItems(items: MentionItem[], query: string): MentionItem[] {
  ...
  const matches = items.filter((it) => it.path.toLowerCase().includes(q));
  matches.sort((a, b) => {
