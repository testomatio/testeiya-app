...
// import.meta.dirname, not import.meta.dir: bunosh runs on Bun or Node, and
// contributors here must not need Bun.
const REPO_ROOT = import.meta.dirname;
const SKILLS_OUTPUT = join(REPO_ROOT, "skills");
const SKILLS_MANIFEST = join(SKILLS_OUTPUT, "skills.yaml");
const SKILLS_LOCK = join(SKILLS_OUTPUT, "skills.lock.json");
...
/**
 * Update the vendored external skills declared in skills/skills.yaml into the
 * committed tree skills/<vendor>/ — one folder per vendor (the repo name,
 * or the owner when the repo is just "skills"). Marketplace categories nest as
 * skills/<vendor>/<category>/<skill>/ (so testomatio/skills →
 ...
 * repo is vendored flat as skills/<vendor>/SKILL.md. Only manifest-owned
 * folders are replaced — any other folder in skills/ (e.g. testeiya/, this
 ...
 * resolved SHAs + owned folders are pinned in skills/skills.lock.json. Re-run on
 * release.
...
/**
 * Scaffold a new first-party skill at skills/testeiya/<name>/SKILL.md and
...
/** List the skills tree (skills/) grouped by vendor; folders not managed by the lock are marked internal. */
 ...
 * to an agent — landing in skills/testeiya/testomatio-docs/docs/ next to a
...
// The same boilerplate the in-app "Write skill" action starts from — one source
// of truth, so a skill scaffolded here and one written in the UI match.
// Kept in step with the same boilerplate in the harness's skills loader, which
// serves the app's "Write skill" editor. Two copies of a template string is the
// price of this repo standing alone.
